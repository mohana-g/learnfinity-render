...
// the new code for profile with achievements
import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import axios from "axios";
import {
...
const LearnerProfileSkeleton = () => (
  <div className="learner-profile-container two-column-layout">
...
const LearnerProfile = () => {
      ...
      try {
        ...
        if (Array.isArray(enrolled) && enrolled.length > 0) {
          ...
          const highQuizScores = enrolled.reduce(
            (acc, c) =>
              acc + ((c.topQuizScores || []).filter((s) => s >= 90).length || 0),
            0
          );
  ...
  const handleSave = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        "https://hilms.onrender.com/api/learner/profile/update",
        editData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="learner-profile-container two-column-layout">
      ...
      <div className="left-section">
        <div className="learner-profile-card">
          {userBadge && (
            <div className={`rank-badge ${userRank ? `rank-${userRank}` : ""}`}>
              <Medal className="icon" /> {userBadge}
            </div>
          )}
          <User className="profile-icon" size={50} />
          {!isEditing ? (
            <>
              <p>
              ...
              <div className="btn-group">
                ...
                <button onClick={handleEditAccount}>Account</button>
              </div>
            </>
          ) : (
            <>
        ...
        {courseProgress.length > 0 && (
          <div className="learner-course-progress">
            <h2>
              <BookOpen className="icon" /> My Course Progress
            </h2>
            {courseProgress.map((course, i) => (
              <div key={i} className="course-progress-card">
                <h4>{course.courseTitle}</h4>
                <div className="progress-bar">
                  <div
                    className="progress-fill"
                ...
                <p>{course.progressPercent}% completed</p>
              </div>
            ))}
          </div>
        )}
