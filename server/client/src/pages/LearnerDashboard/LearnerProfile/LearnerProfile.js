...
const LearnerProfile = () => {
    ...
    const fetchProfileAndProgress = async () => {
      try {
        ...
        const [profileRes, progressRes] = await Promise.all([
          axios.get("http://localhost:5000/api/learner/profile", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get("http://localhost:5000/api/learner/progress", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.put("http://localhost:5000/api/learner/profile/update", editData, {
        headers: {
